...
from models import images, registration
...
class Registration(GUI):
    def __init__(self, parent, controller):
        ...
        frame1.place(rely=0.05, relx=0.02, height=520, width=400)

        frame2 = LabelFrame(self, style.frame_styles, text="Registro")
        frame2.place(rely=0.05, relx=0.45, height=600, width=550)
        
        frame3 = LabelFrame(self, style.frame_styles, text="Configuración de registro")
        frame3.place(rely=0.85, relx=0.02, height=80, width=400)

        label1 = Label(frame1, text= "Selecciona la imagen plantilla", justify=CENTER, **style.frame_styles)
        ...
        label2 = Label(frame1, text= "Selecciona la imagen a transformar", justify=CENTER, **style.frame_styles)
        label2.pack(side=TOP, fill=X, expand=True, padx=5, pady=3)  

        self.combobox_template = Combobox(frame1, font= style.FONT, state= 'readonly')
        self.combobox_template.pack(side=TOP, expand=True, fill=BOTH, padx=5, pady=3)
        
        label3 = Label(frame1, text= "Vista", justify=CENTER, **style.frame_styles)
        ...
        self.canvas_source = Canvas(frame1, bg='white', width=200, height=190)
        self.canvas_source.pack(side=LEFT )
        
        self.canvas_template = Canvas(frame1, bg='white', width=200, height=190)
        self.canvas_template.pack(side=LEFT )                 
        ...
        self.segmented_label = Label(frame2, text= "Imagen registrada", justify=CENTER, **style.frame_styles,)
        ...
        self.bt_segmentation = Button(frame3, **style.button_style, text="Registrar")
        self.bt_segmentation.pack(side=BOTTOM, expand=True, fill=X, padx=5, pady=3)

        
        call_methods_source = images.Images(self.combobox_views, self.combobox_imgs, self.slider, self.canvas_source)
        self.combobox_imgs['values'] =call_methods_source.load_data_combobox("combobox_imgs")
        self.combobox_imgs.current(0)
        self.combobox_imgs.bind("<<ComboboxSelected>>", lambda event: call_methods_source.load_img() )
        
        call_methods_template = images.Images(self.combobox_views, self.combobox_template, self.slider, self.canvas_template)
        self.combobox_template['values'] =call_methods_template.load_data_combobox("combobox_imgs")
        self.combobox_template.current(0)
        self.combobox_template.bind("<<ComboboxSelected>>", lambda event: call_methods_template.load_img() )
        self.slider['command'] = lambda var: (call_methods_source.update_slide(var), call_methods_template.update_slide(var))
        self.combobox_views['values']=call_methods_template.load_data_combobox("combobox_views")
        self.combobox_views.current(0)
        self.combobox_views.bind("<<ComboboxSelected>>", lambda event: (call_methods_template.show_img(self.slider.get()), call_methods_source.show_img(self.slider.get())) )
        ...
        self.bt_segmentation['command'] = lambda : (registration.Registration.register(self.combobox_imgs.get(), self.combobox_template.get(), call_methods_seg), 
                                                    call_methods_seg.set_slider(), call_methods_seg.show_img(self.slider_segmented.get()))
        img_button['command'] = lambda : call_methods_seg.save_image_registration("Registration")
