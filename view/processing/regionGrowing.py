...
class RegionGrowing(GUI):
    def __init__(self, parent, controller):
        ...
        frame2 = LabelFrame(self, style.frame_styles, text="Segmentación con crecimiento de regiones")
        frame2.place(rely=0.05, relx=0.45, height=600, width=550)
        
        frame3 = LabelFrame(self, style.frame_styles, text="Configuración del crecimiento de regiones")
        ...
        self.segmented_label = Label(frame2, text= "Imagen Segmentada", justify=CENTER, **style.frame_styles,)
        ...
        Label(frame3, text="X",  font=style.FONT, bg=style.BG, fg=style.FG).place(x=30, y=10, height=20)
        Entry(frame3, textvariable=self.point_x).place(x=50, y=10, height=20, width=70,)
        Label(frame3, text="Y", font=style.FONT, bg=style.BG, fg=style.FG).place(x=130, y=10, height=20)
        Entry(frame3, textvariable=self.point_y).place(x=150, y=10, height=20, width=70)
        Label(frame3, text="Z", font=style.FONT, bg=style.BG, fg=style.FG).place(x=230, y=10, height=20)      
        Entry(frame3, textvariable=self.point_z).place(x=250, y=10, height=20, width=70)
        Label(frame3, text="Tolerancia", font=style.FONT, bg=style.BG, fg=style.FG).place(x=30, y=40, height=20)        
        Entry(frame3, textvariable=self.tol).place(x=110, y=40, height=20, width=50)
        Label(frame3, text="Profundidad", font=style.FONT, bg=style.BG, fg=style.FG).place(x=160, y=40, height=20)        
        Entry(frame3, textvariable=self.depth).place(x=250, y=40, height=20, width=50)
        Label(frame3, text="Slide", font=style.FONT, bg=style.BG, fg=style.FG).place(x=290, y=40, height=20)        
        Entry(frame3, textvariable=self.slide).place(x=330, y=40, height=20, width=50)

        self.bt_segmentation = Button(frame3, **style.button_style, text="Segmentar")
        self.bt_segmentation.place(x=30, y=70, width=330)
        ...
        self.bt_segmentation['command'] = lambda: (segmentation.Segmentation().regionGrowing(call_methods.mri_image, self.tol.get(), self.point_x.get(), self.point_y.get(), self.point_z.get(), self.depth.get(), self.slide.get(), call_methods_seg), 
                                                    call_methods_seg.set_slider(), call_methods_seg.show_img(self.slider_segmented.get()))
        img_button['command'] = lambda : call_methods_seg.save_image("RegionGrowing")
