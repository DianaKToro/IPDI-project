...
class Kmeans(GUI):
    def __init__(self, parent, controller):
        ...
        frame2 = LabelFrame(self, style.frame_styles, text="Segmentación con K-means")
        frame2.place(rely=0.05, relx=0.45, height=600, width=550)
        
        frame3 = LabelFrame(self, style.frame_styles, text="Configuración de K-means")
        ...
        Label(frame3, text="Valor de k", font=style.FONT, bg=style.BG, fg=style.FG). pack(side=TOP, expand=True, fill=X)
        Entry(frame3, textvariable=self.k_value).pack(side=TOP)
        Label(frame3, text="Iteraciones", font=style.FONT, bg=style.BG, fg=style.FG). pack(side=TOP, expand=True, fill=X)
        Entry(frame3, textvariable=self.iterations).pack(side=TOP)
        ...
        self.bt_segmentation['command'] = lambda : (segmentation.Segmentation.kmeans(call_methods.mri_image, self.k_value.get(), self.iterations.get(), call_methods_seg), 
                                                    call_methods_seg.set_slider(), call_methods_seg.show_img(self.slider_segmented.get()))
        img_button['command'] = lambda : call_methods_seg.save_image("Kmeans")
